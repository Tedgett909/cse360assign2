...
public class interestList<T> extends ArrayList<T> implements OrderedListADT<T>, Serializable 
{
	...
	int object = 0;
	 ...
	 public interestList(int initialCapacity)
	    {
	        rear = 0;
	 ...
	 protected void expandCapacity() //increase size of list in case old list is too small by doubling
	    ...
	    public T remove(T element) //element defined is removed and returned
	    {
	        ...
	        if (index == NOT_FOUND)
	            throw new ElementNotFoundException("interestList");
    ...
    {
        return new interestListIterator();
    }

	private class interestListIterator implements Iterator<T> // implements Iterator for memberList Iterator
	{
		int iteratorModCount;
		int current;
		
		public interestListIterator()
		{
